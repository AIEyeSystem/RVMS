...
class GeometricalVBMs:
    ...
    # """
        ...
        end_points,inter_points = get_terminals_hitmiss(segmentation_skeleton,only_points=1)

        eimg = np.zeros_like(segmentation_skeleton)
        iimg = np.zeros_like(segmentation_skeleton)
        for p in end_points:
            eimg[p[0],p[1]] = 1
        for p in inter_points:
            iimg[p[0],p[1]] = 1        
        return len(end_points), len(inter_points), eimg, iimg
