...
def findClosest(s, p):
    if not s:
        return None  # Return None for empty list s
    
    closest_point = s[0]
    min_distance = distsq(p, s[0])
...
def buildTree(skel, visited, dmap, p, junctions, terminals, G):
    ...
    for n in neighbors:
        ...
        if t in ('j', 't', 'l', 's'):
            ...
            G[p][pp]['cut'] = False #to mark cut nodes in superposition cases
        if t == 'j' or t == 's':
...
def measureApicalDist(G):
    for n in G:
        ...
        r = 1000000
        for n2 in G:
            if G.nodes[n2]['level'] == 1 and n2 != n:
                r = min(r, dist(n, n2))
        G.nodes[n]['apicaldist'] = r
...
def cleanup(G, root, minLength=5, minDia=8, maxRemoveLen = 20):
    ...
    for n in G:
        if G.degree(n) == 1 and n != root:
            ...
            if G[n][p]['branchlength'] < minLength or (G.nodes[n]['dia'] < minDia and G[n][p]['branchlength'] < maxRemoveLen):
                removed.append(n)
...
def plot_graph(G,nodes=None,edges=None,node_labels=None,edge_labels=None,rad=[]):
    ...
    for p in G.nodes:
        pos[p] = p
        #nlabels[p] = "%.1f"%G.nodes[p]['dia']
        nlabels[p] = "%d"%G.nodes[p]['Strahler']
        rad.append(G.nodes[p]['dia'] * .5)
        
    for e in G.edges():
        elabels[e] = G.edges[e[0],e[1]]['Strahler']
        # elabels[e] = G.edges[e[0],e[1]]['level']
        # if G.edges[e[0],e[1]]['alpha'] == None:
        #    elabels[e] = '-'
        # else:
        #    elabels[e] = "%.0f"%(G.edges[e[0],e[1]]['alpha']*180/np.pi) 
    
    nodes = nx.draw_networkx_nodes(G, pos, node_color = [G.nodes[p]['node_color'] for p in G])
...
def report(G,root,px_mm,basename='tmp'):
    ...
    for k in edge_keys:
        if k == 'alpha' or k == 'alpha_e':
            ...
            for e in G.edges(data=True):
                v = e[2][k]
                if e[0] != root and e[1] != root and e[2]['cut'] == False and e[2]['cut'] == False:
                    # this edge does not go to the root
                    # and is not a cut edge
    ...
    for k in all_keys:
        ...
        if(len(data_f['dia']) >0):
            ## data_f is a dict. but the value in data_f is empty array..
...
def addNodeSkel(G,p,skel,junctions,terminals,dmap):
    ...
    d = 2*np.sqrt(dmap[y][x]) 
    G.nodes[p]['dia'] = d
    G.nodes[p]['dia_2'] = d # must be changed
    G.nodes[p]['node_color'] = node_color
    G.nodes[p]['fertile'] = False
    G.nodes[p]['cut'] = False
...
def measureLeafWidth(shape,G,root):
    for n in G:
        if n == root: #For the time being we take the old diameter for the root.
            G.nodes[n]['dia_2'] = G.nodes[n]['dia'] 
        else:
            p = G.nodes[n]['parent']
            #print('node (%5.1f,%5.1f)'%n)
            #print('parent(%5.1f,%5.1f)'%p)
            s1,s2 = cutSpace(shape,n,p)
            G.nodes[n]['dia_2'] = distToSet(s1,n) + distToSet(s1,n)
...
def findCone(shape,p,alpha,d_alpha,eps):
    m1,n1 = lineEqAngle(p,alpha-d_alpha)
    m2,n2 = lineEqAngle(p,alpha+d_alpha)
    s = []
    for (x_n, y_n) in shape:
        if y_n > m1*x_n + n1 and y_n < m2 * x_n + n2 and dist(p,(x_n,y_n)) > eps:
            s.append((x_n, y_n))
    return s
...
def finalStrat(img,contours,alpha,d_alpha,root,eps):
    shape,shape_all = contourToList(contours)
    cutPoints = []
    for i in range(1,len(shape)):
        pClose = findClosest(shape[i],root)
        p = findFurthest(shape[i],pClose)
        if(pClose[1] < p[1]):
            k = findCone(shape_all,p,alpha,d_alpha,eps)
        else:
            k = findCone(shape_all,p,-alpha,d_alpha,eps)
        q = findClosest(k,p)
        img = colorLine(img,p,q)
        cutPoints.append((p,q))

    return img, cutPoints

#Bresenham's algorithm to find pixels in a line between two points
...
def markCutPoints(G,cutPoints,eps):
    for i in cutPoints:
        p,q = i
        for n in G:
            if (dist(n,p) < eps) or (dist(n,q) < eps):
                G.nodes[n]['cut'] = True
                G.nodes[n]['node_color'] = node_color_cut

def find_root_based_optic(end_points,cirle_center=(417,196)):
    #### end_points(y,x),center(x,y)????
    import math
    # 给定点的坐标
    cx, cy = cirle_center
    # 列表中的点（元组）
    ...
    # 打印最近点的下标和对应的距离
    print(f"The index of the nearest point to ({cx}, {cy}) is {nearest_point_index} with a distance of {min_distance}.")
    return end_points[nearest_point_index]
...
def buildGraph(img, skel,root,t,j):
    print('Building tree...')
    G = nx.Graph()
    ...
    contour = findContour(skel)
    # print('valid point in skel: ',np.sum(skel))
    ...
    removed =  cleanup(G, root, minLength=3, minDia=1, maxRemoveLen = 20)
...
# def loopSolution(G,skel,img,contours,root):
...
if __name__ == '__main__':
    ...
    #     imgray = mh.colors.rgb2gray(img, dtype=np.uint8)
    if len(img.shape) == 3:
    ...
    # skel = morphology.skeletonize(img,method='lee') ## (0-255)
    ...
    # input_image = dst.astype(np.uint8) # must be black and white thin network image
    # binary_skel = np.where(skel==0,0,1).astype(np.uint8)

    from skeleton import sk_skeletonize,filter_over
    from terminals import get_terminals_hitmiss,show_terminals
    from plantcv import plantcv as pcv
    from main_branch import two_main_branch,remove_optic,optic_center

    print('Thinning...')
    # skeletonization from scikit image.
    # Zhang-Suen algorithm (apparently with staircase removal)
    # skel = morphology.skeletonize(img,method='lee') ## (0-255)
    # skel = sk_skeletonize(img)
    skel = morphology.skeletonize(img)
    skel = filter_over(skel)
    pruned_skel,segmented_img, segment_objects = pcv.morphology.prune(skel_img=skel, size=15)

    optic_mask_pth = '../Results/M2/optic_disc_cup/raw/1.png'
    optic_mask = cv2.imread(optic_mask_pth,0)
    skel_no_optic = remove_optic(pruned_skel,optic_mask)
    branchs = two_main_branch(skel_no_optic)

    binary_skel = branchs[0]

    cv2.imwrite('binskel.png',binary_skel)

    contours, hierarchy = cv2.findContours(binary_skel, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    print(binary_skel.shape,binary_skel.dtype)
    t,j = get_terminals_hitmiss(binary_skel)

    t = [(p[0],p[1]) for p in t]
    j = [(p[0],p[1]) for p in j]


    terminal_img = show_terminals(binary_skel,t,j)
    cv2.imwrite('terminal.png',terminal_img)
    ...
    img = binary_skel.copy()
    skel = binary_skel.copy()
    skel2 = binary_skel.copy()
    ...
    def updateCircles(G,C):
        for p in C:
            if p not in G:
                ...
                C[p].set_visible(False)
                # REVIEW DIA2 TO DIA
                C[p] = plt.Circle(p, radius=G.nodes[p]['dia_2']/2, alpha=terminal_disk_alpha, color=terminal_disk_color)
    ...
    def keypress(event):
        ...
        if event.key == 'n': #add node
            ...
            Cercle[p] = plt.Circle(p, radius=(G.nodes[p]['dia']/2), alpha=terminal_disk_alpha, color=terminal_disk_color)
            updateCircles(G,Cercle)
            updateMeasures(G, root)
            report(G)
            plot_graph(G)        
        ...
        if event.key=='a': #marking of fertile nodes
            ...
            undo_stack.append((G.copy(),root,Cercle))
            G.nodes[p]['node_color'] = node_color_fertile
            G.nodes[p]['fertile'] = True
            G[p][G.nodes[p]['parent']]['fertile'] = True
            report(G)
            plot_graph(G)
            fig.canvas.draw()
        ...
        if event.key == 'alt+e': #hide any latest selection
            ...
            undo_stack.append((G.copy(),root,Cercle))
            for p in G:
                if G.nodes[p]['node_color'] != node_color:
                    G.nodes[p]['node_color'] = node_color
                    plot_graph(G)
                    fig.canvas.draw()
            report(G)
        ...
        if event.key == 'b': # diameter modification
            ...
            print('closest node is (%5.1f, %5.1f)'%p)
            undo_stack.append((G.copy(),root,Cercle))
            G.nodes[p]['dia_2'] = 2*dist(p,(event.xdata,event.ydata))
            updateMeasures(G, root)
            Cercle[p].remove()
            Cercle[p] = plt.Circle(p,radius=G.nodes[p]['dia_2']/2,alpha=terminal_disk_alpha,color=terminal_disk_color)
            plt.gca().add_patch(Cercle[p])
            plot_graph(G)        
            fig.canvas.draw()   
    ...
    # for n in G:
    #    if G.nodes[n]['level'] == 1:
    #        r = G.nodes[n]['apicaldist']
    ...
    with open(path_basename+'_graph.pkl') as f:
        # nx.write_gpickle(G, )
